...
export function useBorrowingPower(
  ...
  const availableLiquidity = fromLamports(reserve?.info.liquidity.availableAmount, liquidityMintInfo);

  const quoteMintAddess = market?.info?.quoteTokenMint?.toBase58();
